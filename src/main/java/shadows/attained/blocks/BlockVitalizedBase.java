...
public class BlockVitalizedBase extends Block implements IVitalizedSoil, ITOPInfoProvider {
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand,
		...
		if (!world.isRemote) {
			...
			if (heldItem != ItemStack.EMPTY) {
				if (canPlayerEnrich(player, state)) {
					world.setBlockState(pos,
							AD2Util.getBlockFromSoil(AD2Util.getSoilFromItem(heldItem)).getDefaultState(), 2);
					world.playSound((EntityPlayer) null, pos, SoundEvents.BLOCK_GRASS_BREAK, SoundCategory.BLOCKS,
							(float) 0.6, (float) 0.8);
			...
			if (player.getHeldItemMainhand() == ItemStack.EMPTY) {
				if (getBulb() != null) {
					player.sendMessage(new TextComponentString(I18n.format("phrase.attaineddrops.dirtstart")
							+ getBulb().getTextColor() + " " + AD2Util.getBulbDrop(getBulb()).getDisplayName()));
				} else {
	...
	private boolean canPlayerEnrich(EntityPlayer player, IBlockState state) {
		...
		return currentItem != null && AD2Util.isItemApplicable(currentItem)
				&& !AD2Util.isSoilEnriched(AD2Util.getSoilFromBlock(state.getBlock()));
	}
	...
	public void addInformation(ItemStack stack, EntityPlayer player, List<String> list, boolean useExtraInformation) {
		if (!AD2Util.isSoilEnriched(this)) {
			...
			list.add(I18n.format("tooltip.attaineddrops.enrichedwith",
					getBulb().getTextColor() + AD2Util.getBulbDrop(getBulb()).getDisplayName() + TextFormatting.GRAY));
		}
	}
	...
	public void addProbeInfo(ProbeMode mode, IProbeInfo probeInfo, EntityPlayer player, World world,
		...
		if (!AD2Util.isSoilEnriched(this)) {
			if (player.isSneaking() || Keyboard.isKeyDown(AD2Util.getSneakKey().getKeyCode())) {
				...
				probeInfo.horizontal(new LayoutStyle().alignment(ElementAlignment.ALIGN_CENTER)).text(I18n.format(
						"tooltip.attaineddrops.enableditems", TextFormatting.ITALIC + "" + TextFormatting.UNDERLINE));
