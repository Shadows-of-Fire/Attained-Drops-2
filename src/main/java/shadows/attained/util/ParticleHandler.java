	...
	public static void handle(ParticleMessage msg) {
		...
		if (type == 0) {
			...
			while (k < 30) {
				...
				Random rand = Minecraft.getInstance().level.random;
				EndRodParticle p = (EndRodParticle) Minecraft.getInstance().particleEngine.createParticle(ParticleTypes.END_ROD, pos.getX() + rand.nextDouble(), pos.getY(), pos.getZ() + rand.nextDouble(), 0, j, 0);
				p.setColor(msg.color);
				p.setFadeColor(msg.color);
				Minecraft.getInstance().particleEngine.add(p);
			}
