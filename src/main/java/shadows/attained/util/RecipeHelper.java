...
public class RecipeHelper {
	...
	public static void addRecipe(int j, IRecipe rec) {
		CraftingManager.func_193372_a(new ResourceLocation(AttainedDrops2.MODID, "recipes" + j), rec);
	}
	...
	public static ShapedRecipes genShaped(ItemStack output, int l, int w, Object[] input) {
		if (l * w != input.length)
			throw new UnsupportedOperationException(
					"Attempted to add invalid shaped recipe.  Complain to the author of  " + AttainedDrops2.MODNAME);
	...
	public static NonNullList<Ingredient> createInput(Object[] input) {
		...
		for (int i = 0; i < input.length; i++) {
			...
			if (k instanceof String) {
				...
				throw new UnsupportedOperationException(
						"Attempted to add invalid shapeless recipe.  Complain to the author of  " + AttainedDrops2.MODNAME);
			}
		}
		return inputL;
	}
